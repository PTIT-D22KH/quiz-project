...
def quiz_result_view(request, quiz_id, quiz_result_id):
    ...
    if request.method == 'POST':
        ...
        quiz_result.submission_time = timezone.now()
        quiz_result.save()
...
#             description=quiz_description,
#             category_id=quiz_category,
#             total_questions=len(selected_questions)  # Update total_questions
#         )

#         # Update the quiz_id for selected questions
#         Question.objects.filter(id__in=selected_questions).update(quiz_id=quiz)

#         messages.success(request, '')
#         return redirect('all_quiz')  # Redirect to some view after creating the quiz
